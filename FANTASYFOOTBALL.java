...
public class FANTASYFOOTBALL {
    ...
    public static String[] setManagingTeam() throws IOException {
        ...
        String fileName = "teamSheet.txt";
        BufferedReader teamSheet = new BufferedReader (new FileReader(fileName));
        System.out.println("Please select your XI:");
        for (int i = 0; i < 11; i++) {
            System.out.println("Who is your " + position[i] + "?");
            String squadPlayer = teamSheet.readLine();
            player[i] = squadPlayer;
            System.out.println(squadPlayer);
        }
        return player;
    }

    public static void setSquads(teams[] team) throws IOException{
        String[] player = setManagingTeam();
        System.out.println();
    ...
    public static teams[] initialiseTeams(teams managingTeam) throws IOException{
    ...
    public static void printFinalScore(String homeTeam, int homeScore, int awayScore, String awayTeam,
        ...
        for (int i = 0; i < Math.max(homeScore, awayScore); i++) {
            if (i < homeScore && homeGoalScorers[i] != null) {
                ...
                System.out.print(" ".repeat(Math.max(0, maxHomeGoalLength)) + "| ");
            }
    ...
    public static void printFinalScoreFile(String homeTeam, int homeScore, int awayScore, String awayTeam,
                                       int[] homeGoalTime, String[] homeGoalScorers, int[] awayGoalTime, String[] awayGoalScorers, PrintWriter p) {
        p.println();
        p.println();
        p.println("FINAL SCORE: ");
        p.println(homeTeam + " " + homeScore + " - " + awayScore + " " + awayTeam);
        ...
        for (int i = 0; i < Math.max(homeScore, awayScore); i++) {
            if (i < homeScore && homeGoalScorers[i] != null) {
                ...
                p.print(" ".repeat(Math.max(0, maxHomeGoalLength)) + "| ");
            }

            if (i < awayScore && awayGoalScorers[i] != null) {
                p.print(awayGoalTime[i] + "' " + awayGoalScorers[i]);
            } else {
                p.print("           ");
            }
            p.println();
    ...
    public static void testAction(teams homeTeam, teams awayTeam, PrintWriter p) {
        int homeScore = 0;
        int awayScore = 0;
        ...
        printFinalScore(getName(homeTeam), homeScore, awayScore, getName(awayTeam), homeGoalTime, homeGoalScorers,
                awayGoalTime,
                awayGoalScorers);
        printFinalScoreFile(getName(homeTeam), homeScore, awayScore, getName(awayTeam), homeGoalTime, homeGoalScorers,
                awayGoalTime,
                awayGoalScorers, p);
    ...
    public static void testBackgroundAction(teams homeTeam, teams awayTeam) {
        ...
        for (int minute = 1; minute <= 90; minute++) {
            for (int actionWithinMinute = 1; actionWithinMinute <= 3; actionWithinMinute++) {
    ...
    // Writes the header for the table to a file.
    public static void printTableHeaderFile(PrintWriter p) {
        p.println(
                "---------------------------------------------------------------------------------------------------------------------------------------------");
        p.printf("| %-6s | %-25s | %-12s | %-6s | %-6s | %-9s | %-12s | %-12s | %-11s | %-6s |\n", "Pos.",
                "Team", "Games Played", "Wins", "Draws", "Losses", "Goals For", "Goals Against", "Goal Diff.",
                "Points");
        p.println(
                "---------------------------------------------------------------------------------------------------------------------------------------------");
    ...
    // Prints the statistics for a specific team in the table.
    ...
    // Writes the statistics for a specific team to a file.
    public static void printTeamStatsFile(teams team, int i, PrintWriter p) {
        p.printf("| %-6d | %-25s | %-12d | %-6d | %-6d | %-9d | %-12d | %-12d | %-11d | %-6d |\n",
                i, getName(team), getPlayed(team), getWins(team), getDraws(team), getLosses(team), getGoalsFor(team),
                getGoalsAgainst(team),
                (getGoalsFor(team) - getGoalsAgainst(team)), getPoints(team));
    }
    ...
    // Writes the footer for the table to a file.
    public static void printTableFooterFile(PrintWriter p) {
        p.println(
                "---------------------------------------------------------------------------------------------------------------------------------------------");
    }

    // Prints the table with teams sorted based on points, goal difference, and goals scored. Additionally, writes the table to a file.
    public static void printTable(teams[] team) throws IOException{
        teams[] sortedTeams = Arrays.copyOf(team, team.length);
        ...
        PrintWriter tableFile = new PrintWriter (new FileWriter("tableFile.txt"));
        printTableHeader();
        printTableHeaderFile(tableFile);

        for (int i = 0; i < sortedTeams.length; i++) {
            teams t = sortedTeams[i];
            printTeamStats(t, (i + 1));
            printTeamStatsFile(t, (i + 1), tableFile);
        }

        printTableFooter();
        printTableFooterFile(tableFile);

        tableFile.close();
    }

    public static void matchSimulation(int matchday, teams oppositionTeam, teams managingTeam, PrintWriter p) {
        System.out.println("Matchday " + matchday + ": You will be facing " + getName(oppositionTeam) + ".");
        testAction(managingTeam, oppositionTeam, p);
        System.out.println();
        System.out.println();
    }

    public static void matchAwaySimulation(int matchday, teams managingTeam, teams oppositionTeam, PrintWriter p) {
        System.out.println("Matchday " + matchday + ": You will be facing " + getName(oppositionTeam) + ".");
        testAction(oppositionTeam, managingTeam, p);
        System.out.println();
        System.out.println();
    }
    ...
    public static void homeSeries(teams[] team, teams managingTeam, PrintWriter p) {
        // GW1
        matchSimulation(1, team[1], managingTeam, p);
        ...
        continueKey();
        // GW2
        matchSimulation(2, team[2], managingTeam, p);
        ...
        continueKey();
        // GW3
        matchSimulation(3, team[3], managingTeam, p);
        otherGames(1, 2, team);
        ...
        continueKey();
        // GW4
        matchSimulation(4, team[4], managingTeam, p);
        ...
        continueKey();
        // GW5
        matchSimulation(5, team[5], managingTeam, p);
        otherGames(1, 3, team);
        otherGames(2, 4, team);
        ...
        continueKey();
        // GW6
        matchSimulation(6, team[6], managingTeam, p);
        ...
        continueKey();
        // GW7
        matchSimulation(7, team[7], managingTeam, p);
        otherGames(1, 4, team);
        otherGames(2, 6, team);
        otherGames(3, 5, team);
        ...
        continueKey();
        // GW8
        matchSimulation(8, team[8], managingTeam, p);
        ...
        continueKey();
        // GW9
        matchSimulation(9, team[9], managingTeam, p);
        otherGames(1, 5, team);
        otherGames(2, 8, team);
        otherGames(3, 7, team);
        otherGames(4, 6, team);
        ...
        continueKey();
        // GW10
        matchSimulation(10, team[10], managingTeam, p);
        ...
        continueKey();
        // GW11
        matchSimulation(11, team[11], managingTeam, p);
        otherGames(1, 6, team);
        ...
        // GW12
        matchSimulation(12, team[12], managingTeam, p);
        ...
        continueKey();
        // GW13
        matchSimulation(13, team[13], managingTeam, p);
        otherGames(1, 7, team);
        ...
        continueKey();
        // GW14
        matchSimulation(14, team[14], managingTeam, p);
        ...
        continueKey();
        // GW15
        matchSimulation(15, team[15], managingTeam, p);
        otherGames(1, 8, team);
        ...
        continueKey();
        // GW16
        matchSimulation(16, team[16], managingTeam, p);
        ...
        continueKey();
        // GW17
        matchSimulation(17, team[17], managingTeam, p);
        otherGames(1, 9, team);
        ...
        continueKey();
        // GW18
        matchSimulation(18, team[18], managingTeam, p);
        ...
        continueKey();
        // GW19
        matchSimulation(19, team[19], managingTeam, p);
    ...
    public static void awayTour(teams[] team, teams managingTeam, PrintWriter p) {
        // GW20
        matchAwaySimulation(20, managingTeam, team[19], p);
        ...
        continueKey();
        // GW21
        matchAwaySimulation(21, managingTeam, team[18], p);
        ...
        continueKey();
        // GW22
        matchAwaySimulation(22, managingTeam, team[17], p);
        ...
        continueKey();
        // GW23
        matchAwaySimulation(23, managingTeam, team[16], p);
        ...
        continueKey();
        // GW24
        matchAwaySimulation(24, managingTeam, team[15], p);
        ...
        continueKey();
        // GW25
        matchAwaySimulation(25, managingTeam, team[14], p);
        ...
        continueKey();
        // GW26
        matchAwaySimulation(26, managingTeam, team[13], p);
        ...
        continueKey();
        // GW27
        matchAwaySimulation(27, managingTeam, team[12], p);
        ...
        continueKey();
        // GW28
        matchAwaySimulation(28, managingTeam, team[11], p);
        ...
        continueKey();
        // GW29
        matchAwaySimulation(29, managingTeam, team[10], p);
        ...
        continueKey();
        // GW30
        matchAwaySimulation(30, managingTeam, team[9], p);
        ...
        continueKey();
        // GW31
        matchAwaySimulation(31, managingTeam, team[8], p);
        ...
        continueKey();
        // GW32
        matchAwaySimulation(32, managingTeam, team[7], p);
        ...
        continueKey();
        // GW33
        matchAwaySimulation(33, managingTeam, team[6], p);
        ...
        continueKey();
        // GW34
        matchAwaySimulation(34, managingTeam, team[5], p);
        ...
        continueKey();
        // GW35
        matchAwaySimulation(35, managingTeam, team[4], p);
        ...
        continueKey();
        // GW36
        matchAwaySimulation(36, managingTeam, team[3], p);
        ...
        continueKey();
        // GW37
        matchAwaySimulation(37, managingTeam, team[2], p);
        ...
        continueKey();
        // GW38
        matchAwaySimulation(38, managingTeam, team[1], p);
    ...
    public static void main(String[] args) throws IOException {
        ...
        String sf = "scoreFile.txt";
        PrintWriter scoreFile = new PrintWriter (new FileWriter(sf));

        homeSeries(team, team[0], scoreFile);
        awayTour(team, team[0], scoreFile);
